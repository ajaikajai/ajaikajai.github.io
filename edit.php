...
<body class="dashboard-page">
	...
	<section class="wrapper scrollable">
		...
		<section class="title-bar">
			<div class="logo">
				<h1><a href="index.html"><img src="images/logo.png" alt="" />hello</a></h1>
			</div>
...
$edit = $_GET["id"];
$res = $con->query("SELECT * FROM product_id WHERE id = '$edit'");
$count = $res->num_rows;
if ($count > 0) {
    $row = $res->fetch_assoc();
}
?>
<form method="post" action="update.php?id=<?php echo $row['id']; ?>" class="valida" enctype="multipart/form-data" autocomplete="off" novalidate="novalidate">


    <input type="hidden" value="<?php echo($row['id']); ?>"
    <input type="hidden" value="<?php echo ($row['Product Name']); ?>"
    <input type="hidden" value="<?php echo ($row['Price']); ?>"
    <input type="hidden" value="<?php echo ($row['Quandity']); ?>"

  

    <label for="Product Name">Product Name: (required)&nbsp;<span class="at-required-highlight">*</span></label>
    <div class="form-group">
        <input type="text" value="<?php echo ($row['Product Name']); ?>" name="Product Name" class="form-control">
    </div>
    ...
    <label for="Quandity">Quandity: (required)&nbsp;<span class="at-required-highlight">*</span></label>
    <div class="form-group">
        <input type="text" value="<?php echo ($row['Quandity']) ; ?>" name="Quandity" class="form-control">
    </div>

    <div>
        <input type="file" id="fileInput" name="new_image"><br>
        <img id="image" src="prd/<?php echo $row['image']; ?>" width="100" height="100">
    </div>
    ...
    <div>
		...
		<!-- footer -->
		<div class="footer">
			<p>© 2023  . All Rights Reserved . Design by <a href="http://w3layouts.com/">ajai</a></p>
		</div>
		<!-- //footer -->
	</section>
	...
	<!-- input-forms -->
        ...
        // setup the partial validation
				$('#partial-1').on('click', function( ev ) {
					...
					$('form').valida('partial', '#productName'); // validate only productName
					$('form').valida('partial', '#Quandity'); // validate only Quandity
				});
